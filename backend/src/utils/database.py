    ...
    def initialize(self) -> None:
        ...
        for attempt in range(self._max_connection_attempts):
            try:
                ...
                logger.info(
                    f"Initializing database connection pool (attempt {self._connection_attempts}/{self._max_connection_attempts})"
                )
                ...
                self._pool = pooling.MySQLConnectionPool(
                    pool_name="clash_deck_builder_pool", pool_size=10, pool_reset_session=True, **db_config
                )
                ...
                if attempt < self._max_connection_attempts - 1:
                    ...
                    raise ConnectionPoolError(
                        f"Failed to initialize database connection pool after {self._max_connection_attempts} attempts: {e}"
                    )
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.get_connection() as conn:
                ...
                if result:
                    health_status.update({"status": "healthy", "connection_test": "passed"})
